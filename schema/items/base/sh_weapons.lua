...
ITEM.functions.EquipUn = { -- sorry, for name order.
	...
	OnRun = function(item)
		...
		if ( ply.isUnEquippingWeapon ) then
			ply:Notify("You cannot un-equip another weapon right now.")

			return false
		end

		if ( ply.isEquippingWeapon ) then
			ply:Notify("You cannot equip another weapon right now.")

			return false
		end

		if ( ply.isLoadingAmmo ) then
			ply:Notify("You cannot equip a weapon while loading ammo.")

			return false
		end

		if ( item.unEquipTime and item.unEquipTime > 0 ) then
			ply.isUnEquippingWeapon = true

			local success, erorrs = pcall(function()
				ply:SetAction("Equipping Weapon..", item.unEquipTime, function()
					item:Unequip(ply, true)

					ply.isUnEquippingWeapon = false
				end)
			end)
...
ITEM.functions.Equip = {
	...
	OnRun = function(item)
		...
		if ( item.useTime and item.useTime > 0 ) then
			ply.isEquippingWeapon = true

			local success, erorrs = pcall(function()
				ply:SetAction("Equipping Weapon..", item.useTime, function()
					item:Equip(ply)

					ply.isEquippingWeapon = false
				end)
			end)
...
function ITEM:Equip(client, bNoSelect, bNoSound)
	local items = client:GetCharacter():GetInventory():Iter()
...
hook.Add("PlayerDeath", "ixStripClip", function(client)
	...
	for k, v in client:GetCharacter():GetInventory():Iter() do
